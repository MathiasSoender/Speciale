...
namespace Speciale.CSC
{
    public class CSC_v3
    {
        ...
        public Phrase CompressSpecificPhrase(int matched, int curSuffix)
        {
            int predSAIndexed = FindNextPred(curSuffix, matched);
            int succSAIndexed = FindNextSucc(curSuffix, matched);

            return FindBestMatch(predSAIndexed, succSAIndexed, curSuffix, matched);
        }
